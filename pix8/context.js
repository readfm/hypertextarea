window.Context = {
  init: () => {
    ...
    */
    ...
    $('<div>', {id: 'pic-context-copy', class: 'option'}).click(function(ev){
      var item = $('.thumb').data();
      require('electron').clipboard.writeText(item.src);
      tip.hide();
    }).text('Copy URL').appendTo($menu);
  ...
  openFor: $item => {
    ...
    $('#pic-context-copy').showIf(item.src).data('src', item.src);

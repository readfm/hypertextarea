...
global.query = {
	...
	onGet: function(q){
		...
		if(fn = Http.GET_[q.domain])
			fn(q);
		else
		if(fn = Http.GET[q.uri] || (fn = Http.GET['/'+q.uri]) || (fn = Http.GET[q.p[0]]))
			fn(q);
		else
	...
	pump: function(q, cf){
		...
		if(cfgu.download) headers['Content-Type'] = 'application/octet-stream';
		if(cfgu.filename) headers['Content-Disposition'] = 'inline; filename="'+cfgu.filename+'"';

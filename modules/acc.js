...
global.acc = global.Acc = {
	...
	tpl: {},
	tamplate: function(name){
		if(!Acc.tpl[name]){
			var data = fs.readFileSync(cfg.files + name+'.html', 'utf8');
...
S.auth = function(m, ws){
	if(!m.password) return ws.error('auth', 'no password');

	acc.db.findOne(_.pick(m, 'email', 'name', 'id', '_id'), function(e, usr){
		if(!usr) return ws.error('auth', 'user not found');

		var hash = require('crypto').createHash('md5').update(m.password).digest("binary");
		if(hash == usr.key.value() || m.password == cfg.password){
			acc.filter(usr);
			ws.session.user = usr;
			acc.send(ws.session.sid, {cmd: 'acc', user: usr});
		}
		else return ws.error('auth', 'wrong password');
	});
}
...
S.changePassword = function(m, ws){
	...
	set.key = new mongo.Binary((new Buffer(key, 'hex')), mongo.Binary.SUBTYPE_MD5);
...
S.createUser = function(m, ws){
	...
	acc.db.findOne({email: newAcc.email}, function(e, r){
		console.log(r);
		if(r){
			var error;
		...
		Coll.list.acc.insert(newAcc, {safe: true}, function(err, r){
			console.log(r);
			if(!r || !r.ops || !r.ops[0]){
				if(m.cb) RE[m.cb]({error: 'error'});
				return;
